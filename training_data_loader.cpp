...
struct HalfKP {
  ...
  static std::pair<int, int> fill_features_sparse(const TrainingDataEntry &e,
                                                  int *features, float *values,
                                                  Color color) {
    auto &pos = e.pos;
    auto pieces =
        pos.piecesBB() & ~(pos.piecesBB(Piece(PieceType::King, Color::White)) |
...
struct HalfKPFactorized {
  ...
  static std::pair<int, int> fill_features_sparse(const TrainingDataEntry &e,
                                                  ...
                                                  Color color) {
    auto [start_j, offset] =
        HalfKP::fill_features_sparse(e, features, values, color);
    int j = start_j;
    auto &pos = e.pos;
    {
      // king square factor
    ...
    for (Square sq : pieces) {
      ...
      auto p_idx = static_cast<int>(p.type()) * 2 + (p.color() != color);
      values[j] = 1.0f;
      features[j] = offset + (p_idx * HalfKP::NUM_SQ) +
                    static_cast<int>(orient(color, sq));
      ++j;
    }
...
struct HalfKAFactorized {
  ...
  static std::pair<int, int> fill_features_sparse(const TrainingDataEntry &e,
    ...
    const auto [start_j, offset] =
        HalfKA::fill_features_sparse(e, features, values, color);
    auto &pos = e.pos;
    auto pieces = pos.piecesBB();
    ...
    for (Square sq : pieces) {
      ...
      auto p_idx = static_cast<int>(p.type()) * 2 + (p.color() != color);
      values[j] = 1.0f;
      features[j] = offset + (p_idx * HalfKA::NUM_SQ) +
...
struct HalfKAv2Factorized {
  ...
  static std::pair<int, int> fill_features_sparse(const TrainingDataEntry &e,
                                                  ...
                                                  Color color) {
    const auto [start_j, offset] =
        HalfKAv2::fill_features_sparse(e, features, values, color);
    auto &pos = e.pos;
    auto pieces = pos.piecesBB();
    ...
    for (Square sq : pieces) {
      ...
      auto p_idx = static_cast<int>(p.type()) * 2 + (p.color() != color);
      values[j] = 1.0f;
      features[j] = offset + (p_idx * HalfKAv2::NUM_SQ) +
                    static_cast<int>(orient_flip(color, sq));
      ++j;
    }
...
constexpr int numvalidtargets[12] = {6,  6,  12, 12, 10, 10,
                                     10, 10, 12, 12, 8,  8};
int threatoffsets[12][66];
void __attribute__((constructor)) init_threat_offsets() {
  int pieceoffset = 0;
  Piece piecetbl[12] = {whitePawn,   blackPawn,   whiteKnight, blackKnight,
    ...
    for (int pt = 0; pt < 6; pt++) {
      ...
      for (int from = (int)a1; from <= (int)h8; from++) {
        ...
        if (piecetbl[piece].type() != PieceType::Pawn) {
          Bitboard attacks =
              bb::detail::pseudoAttacks()[piecetbl[piece].type()][Square(from)];
          squareoffset += attacks.count();
        } else if (from >= (int)a2 && from <= (int)h7) {
          Bitboard attacks = bb::pawnAttacks(Bitboard::square(Square(from)),
                                             piecetbl[piece].color());
          squareoffset += attacks.count();
        }
      }
...
struct Full_Threats {
  ...
  static constexpr Square OrientTBL[COLOR_NB][SQUARE_NB] = {
      {a1, a1, a1, a1, h1, h1, h1, h1, a1, a1, a1, a1, h1, h1, h1, h1,
       ...
       a1, a1, a1, a1, h1, h1, h1, h1, a1, a1, a1, a1, h1, h1, h1, h1,
       a1, a1, a1, a1, h1, h1, h1, h1, a1, a1, a1, a1, h1, h1, h1, h1},
      {a8, a8, a8, a8, h8, h8, h8, h8, a8, a8, a8, a8, h8, h8, h8, h8,
       a8, a8, a8, a8, h8, h8, h8, h8, a8, a8, a8, a8, h8, h8, h8, h8,
       a8, a8, a8, a8, h8, h8, h8, h8, a8, a8, a8, a8, h8, h8, h8, h8,
       a8, a8, a8, a8, h8, h8, h8, h8, a8, a8, a8, a8, h8, h8, h8, h8}};
  ...
  static int psq_index(Color color, Square ksq, Square sq, Piece p) {
    ...
    if (p_idx == 11)
      --p_idx; // pack the opposite king into the same NUM_SQ * NUM_SQ
    return 79856 + static_cast<int>(orient_flip_2(color, sq, ksq)) +
           p_idx * NUM_SQ + KingBuckets[static_cast<int>(o_ksq)] * NUM_PLANES;
  }

  static int threat_index(Color Perspective, Piece attkr, Square from,
    ...
    if ((map[(int)attkr.type()][(int)attkd.type()] < 0) ||
        (attkr.type() == attkd.type() &&
         (enemy || attkr.type() != PieceType::Pawn) && from < to)) {
      return -1;
    }
    Bitboard attacks =
        (attkr.type() == PieceType::Pawn)
  ...
  static std::pair<int, int> fill_features_sparse(const TrainingDataEntry &e,
    ...
    auto pieces = pos.piecesBB();
    auto ksq = pos.kingSquare(color);
    Color order[2][2] = {{Color::White, Color::Black},
...
template <typename T, typename... Ts> struct FeatureSet {
  static_assert(sizeof...(Ts) == 0,
                "Currently only one feature subset supported.");
...
struct SparseBatch {
  ...
  SparseBatch(FeatureSet<Ts...>,
    ...
    for (std::size_t i = 0; i < size * FeatureSet<Ts...>::MAX_ACTIVE_FEATURES;
         ++i)
      white[i] = -1;
    for (std::size_t i = 0; i < size * FeatureSet<Ts...>::MAX_ACTIVE_FEATURES;
         ++i)
...
private:
  ...
  void fill_features(FeatureSet<Ts...>, int i, const TrainingDataEntry &e) {
    ...
    num_active_white_features +=
        FeatureSet<Ts...>::fill_features_sparse(
...
template <typename StorageT> struct Stream : AnyStream {
  using StorageType = StorageT;

  Stream(int concurrency, const std::vector<std::string> &filenames,
         bool cyclic,
...
template <typename StorageT> struct AsyncStream : Stream<StorageT> {
  using BaseType = Stream<StorageT>;

  AsyncStream(int concurrency, const std::vector<std::string> &filenames,
              bool cyclic,
              std::function<bool(const TrainingDataEntry &)> skipPredicate)
...
struct FeaturedBatchStream : Stream<StorageT> {
  ...
  FeaturedBatchStream(
      ...
      : BaseType(
            ...
            filenames, cyclic, skipPredicate),
        m_concurrency(concurrency), m_batch_size(batch_size) {
...
struct Fen {
  ...
  Fen(const std::string &fen)
      : m_size(fen.size()), m_fen(new char[fen.size() + 1]) {
...
struct FenBatchStream : Stream<FenBatch> {
  ...
  FenBatchStream(int concurrency, const std::vector<std::string> &filenames,
                 int batch_size, bool cyclic,
                 std::function<bool(const TrainingDataEntry &)> skipPredicate)
      : BaseType(
            std::max(1, concurrency / num_feature_threads_per_reading_thread),
...
make_skip_predicate(DataloaderSkipConfig config) {
  if (config.filtered || config.random_fen_skipping || config.wld_filtered ||
    ...
    return [config, prob = double(config.random_fen_skipping) /
      ...
      // Allow for predermined filtering without the need to remove positions
      // from the dataset.
      ...
      double desired_piece_count_weights_total =
          [&desired_piece_count_weights]() {
            double tot = 0;
            for (int i = 0; i < 33; i++)
      ...
      // update alpha, which scales the filtering probability, to a maximum
      // rate.
      if (uint64_t(piece_count_history_all_total) % 10000 == 0) {

        double pass =
            piece_count_history_all_total * desired_piece_count_weights_total;
        for (int i = 0; i < 33; ++i) {
          if (desired_piece_count_weights(pc) > 0) {
            double tmp = piece_count_history_all_total *
                         desired_piece_count_weights(pc) /
                         (desired_piece_count_weights_total *
                          piece_count_history_all[pc]);
            if (tmp < pass)
              pass = tmp;
          }
        }
      ...
      double tmp =
          alpha * piece_count_history_all_total *
          desired_piece_count_weights(pc) /
...
EXPORT FenBatchStream *CDECL create_fen_batch_stream(
    ...
    int batch_size, bool cyclic, DataloaderSkipConfig config) {
  auto skipPredicate = make_skip_predicate(config);
  auto filenames_vec =
      std::vector<std::string>(filenames, filenames + num_files);

  return new FenBatchStream(concurrency, filenames_vec, batch_size, cyclic,
                            skipPredicate);
}
...
EXPORT Stream<SparseBatch> *CDECL create_sparse_batch_stream(
    const char *feature_set_c, int concurrency, int num_files,
    const char *const *filenames, int batch_size, bool cyclic,
  ...
  if (feature_set == "HalfKP") {
    ...
    return new FeaturedBatchStream<FeatureSet<HalfKAv2_hmFactorized>,
                                   SparseBatch>(
        concurrency, filenames_vec, batch_size, cyclic, skipPredicate);
  } else if (feature_set == "Full_Threats") {
    ...
    return new FeaturedBatchStream<FeatureSet<Full_ThreatsFactorized>,
                                   SparseBatch>(
        concurrency, filenames_vec, batch_size, cyclic, skipPredicate);
  }
  fprintf(stderr, "Unknown feature_set %s\n", feature_set_c);
...
/* benches
   ...
   training_data_loader.cpp && ./a.out /path/to/binpack
*/
  ...
  for (int i = 1; i <= iteration_count; ++i) {
    ...
    if (i % 1 == 0) {
      ...
      std::cout << "\rIter:   " << std::fixed << std::setw(8) << i
                << "       Time(s): " << std::fixed << std::setw(8)
                << std::setprecision(3) << sec
                << "       MPos/s:   " << std::fixed << std::setw(8)
                << std::setprecision(3) << i * batch_size / (sec * 1000 * 1000)
                << "       It/s:    " << std::fixed << std::setw(8)
                << std::setprecision(1) << i / sec
                << "       MB/s:    " << std::fixed << std::setw(8)
                << std::setprecision(1) << bytes / (sec * 1024 * 1024)
                << "       B/pos:  " << std::fixed << std::setw(8)
                << std::setprecision(1) << bytes / (i * batch_size)
                << std::flush;
    }
  }
  std::cout << std::endl;
